...
def initialize_Ogre(check_install):
    ...
    if check_install:
        try:
            ...
            result = subprocess.run([sys.executable, "-m", "pip", "show", "OgreInterface"], capture_output=True, text=True)
...
def lattice_matching(CIF_folder, material_A, material_B, max_hkl_A, max_hkl_B, max_strain, max_area, image_dpi, image_scale):
    ...
    misfit_fig_name = f"misfit_plot_{material_A}_{material_B}_max_{max_hkl_A}_{max_hkl_B}.png"
    ms.plot_misfits(dpi=image_dpi, fontsize=14, labelrotation=-45, figure_scale=1, output=misfit_fig_name) # Plot the misfit results
...
def visualize_supercells(CIF_folder, material_A, material_B, hkl_A, hkl_B, mode, image_dpi, max_strain, max_area, image_scale):
    ...
    if isinstance(max_area, (int, float)):
        interface_generator = InterfaceGenerator(substrate=sub, film=film, max_strain=max_strain, max_area=max_area, interfacial_distance=None, vacuum=60, center=True)
    else:
        interface_generator = InterfaceGenerator(substrate=sub, film=film, max_strain=max_strain, interfacial_distance=None, vacuum=60, center=True)
...
def surface_matching(CIF_folder, material_A, material_B, hkl_A, hkl_B, label, max_strain, max_area, strain_fraction, supercell_choice, n_particles_PSO,
    ...
    output_folder_name = material_A + "_" + ''.join(str(x) for x in hkl_A) + "_" + material_B + "_" + ''.join(str(x) for x in hkl_B) + label
    ...
    if mlip is None:
        iface_search = IonicInterfaceSearch(
            ...
            use_most_stable_substrate = False,           # Excludes fragments with high energy (faster, but might miss plausible models)
            substrate_strain_fraction = strain_fraction,             # Distribution of strain between substrate and epilayer (1 = substrate strin only)
...
if __name__ == "__main__":
    ...
    lattice_matching(CIF_folder, material_A, material_B, max_hkl_A, max_hkl_B, max_strain, max_area, image_dpi, image_scale)

    # Execute the supercell visualization process
    visualize_supercells(CIF_folder, material_A, material_B, hkl_A, hkl_B, mode, image_dpi, max_strain, max_area, image_scale)

    # Execute the surface matching process
    surface_matching(CIF_folder, material_A, material_B, hkl_A, hkl_B, label, max_strain, max_area, strain_fraction, supercell_choice, n_particles_PSO)
